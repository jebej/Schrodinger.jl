...
for (syevr1, syevr2, elty, relty) in
    ...
    @static if VERSION < v"0.7.0-"
        @eval begin
            # Real symmetric
            function syevr!(w::Vector{$relty}, Z::Matrix{$relty}, uplo::Char, A::StridedMatrix{$relty},
                ...
                info   = Ref{BlasInt}()
                for i = 1:2
                    ccall((@blasfunc($syevr1), liblapack), Void,
                    (Ptr{UInt8}, Ptr{UInt8}, Ptr{UInt8}, Ptr{BlasInt},
                    Ptr{$relty}, Ptr{BlasInt}, Ptr{$relty}, Ptr{$relty},
                    Ptr{BlasInt}, Ptr{BlasInt}, Ptr{$relty}, Ptr{BlasInt},
                    Ptr{$relty}, Ptr{$relty}, Ptr{BlasInt}, Ptr{BlasInt},
            ...
            # Hermitian
            function syevr!(w::Vector{$relty}, Z::Matrix{$elty}, uplo::Char, A::StridedMatrix{$elty},
                vl::AbstractFloat, vu::AbstractFloat, il::Integer, iu::Integer, abstol::AbstractFloat)
                chkstride1(A)
                n = checksquare(A)
                length(w) == n || throw(ArgumentError("vector w should have length $n."))
                ...
                info   = Ref{BlasInt}()
                for i = 1:2
                    ccall((@blasfunc($syevr2), liblapack), Void,
                    (Ptr{UInt8}, Ptr{UInt8}, Ptr{UInt8}, Ptr{BlasInt},
